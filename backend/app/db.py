...
def init_db():
    ...
    with engine.begin() as conn:
        conn.execute(text("""
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='acts' AND column_name='featured')
  THEN ALTER TABLE acts ADD COLUMN featured boolean DEFAULT false; END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='acts' AND column_name='premium')
  THEN ALTER TABLE acts ADD COLUMN premium boolean DEFAULT false; END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='venues' AND column_name='featured')
  THEN ALTER TABLE venues ADD COLUMN featured boolean DEFAULT false; END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='venues' AND column_name='premium')
  THEN ALTER TABLE venues ADD COLUMN premium boolean DEFAULT false; END IF;
END $$;"""))

def seed_if_needed():
    if os.getenv("SEED","0")!="1": return
    db = SessionLocal()
    try:
        ...
        if not provu:
            ...
            db.add(provu); db.flush(); db.add(Provider(user_id=provu.id, display_name="Neon Nights", location="Newcastle", bio="High-energy party band.", status="approved"))
        ...
        if not bizu:
            ...
            db.add(bizu); db.flush(); db.add(Business(user_id=bizu.id, company="Eventify Ltd", plan="free", lead_credits=3))
        ...
        if db.query(Act).count()==0:
            ...
            from .models import Package, Media, Availability
            db.add_all([
                Package(act_id=band.id, name="Evening Set", price=1200, duration_mins=120, description="2x60min sets, party hits"),
                Package(act_id=band.id, name="All-day", price=2200, duration_mins=300, description="Acoustic afternoon + evening party"),
                Media(act_id=band.id, url=band.image_url, media_type="image", sort=0),
                Availability(act_id=band.id, date="2026-06-18", is_available=True),
            ])
